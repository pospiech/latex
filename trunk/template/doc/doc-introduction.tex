...
as the thesis of teachers in their practical year. These thesis had all a natural science background, which means that also this template is optimized for the needs of people in natural sciences. Nevertheless it should be easily adaptable to topics in humanities, linguistics or even arts.
...
This is achieved mainly by using commands like \cs{IfPackageLoaded}, \cs{IfPackagesNotLoaded}, \cs{ExecuteAfterPackage}, \cs{IfFileExists}, \cs{IfMultDefined} and others mostly defined by the package \package{templatetools.sty}. 
...
It may happens that a bug fix in this template has become obsolete because it was in the meantime fixed in the package. In that case please inform the template maintainer.

% ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
\subsection{Document features}
\label{sec:doc:features:doc}
This template provides all methods (commands, environments, work flows) that are required for a complex scientific document. This is realized by 
loading a large number of relevant and modern packages of \latex. It is difficult to provide a complete list of the resulting features therefore the following lists include only a subset of the most interesting ones.
...
\begin{itemize}
\item Professional math typsetting with a large number of supported symbols and commands using \package{amsmath}, \package{mathtools} and others.
%
...
logarithmic scaling (\package{pgfplots}, \package{pgfplotstable})
\end{itemize}
...
%
\item Table columns with variable width (so called \enquote{X} columns) and linebreak support (\package{tabularx}, \package{tabu}),
%
...
\caption[Execution times of the template divided into compilation steps]{Execution times of the template divided into compilation steps. The largest execution times come from the major packages. The packages loaded in each step are listed in \cref{tab:doc:packages}}
...
The content files are organized in files that belong to the front pages (\emph{frontmatter}):
%
\begin{itemize}
...
\end{itemize}
%
Further files that belong to the main content (\emph{mainmatter}). These should for example be named according to the chapter contents such as: 
%
\begin{itemize}
...
At the beginning, the documents in the front and the end should be adapted to the documents content. For example the users name, institution, title can be inserted. Next the main files should be renamed according to the chapter organization of the document. If certain automatic generated lists such as the index, a glossary or others are not needed these should be disabled in the main file.
...
%% =========================================================================
\chapter{Settings, locations and solutions}
\label{chap:doc:faq}
This chapters contains all sorts of answers to typical questions, locations of settings and general solutions with \latex. Further examples of the possibilities of this template are shown with code and examples in \cref{part:demo}.
...
This template tries to differentiate clearly between functionality (package loading) and configuration of the layout. The first is done primarily in file \file{preamble/package.tex} the latter mainly in file \file{preamble/style.tex}. Nevertheless this separation can not be fully realized and further are the options spread over several files.
...
This is realized with \latex using the command \cs{includeonly}. This list
contains all files loaded with \cs{include} that shall be included in the current compilation. All information on those files not included into the compilation, such as labels, is nevertheless included. This only requires that each files was at least once included in the compilation.
...
The approach of this template is to use the package \package{morewrites}, which
hooks at the lowest level (\TeX primitives) to solve this problem. This package however itself might cause problems since it is not tested extensively. 
