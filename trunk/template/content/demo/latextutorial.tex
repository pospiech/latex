...
For further reading on the possibilities of this template please refer to the file \path{TemplateDocumentation.pdf}.
...
The standard command for citations is \texttt{\textbackslash{}cite} which may have a prenote argument for adding a page number or something similar. To show how a citation is typeset we cite here a book about LaTeX \cite[59]{companion}. Further commands such as \cs{parencite} \parencite{companion} and \cs{textcite} \textcite{companion} allow a different typeset of the citation. The resulting bibliography is printed out on \cpageref{sec:bibliography}. Refer to the biblatex manual for further details on citation commands and modifications on the printout and the section on biblatex in \path{TemplateDocumentation.pdf}.
...
Description lists and other list environments and options for list are described in \path{TemplateDocumentation.pdf}.
...
The design and creating of complex tables is shown in much greater detail in \path{TemplateDocumentation.pdf} in the section about tables.
...
\end{aligned}
\end{equation}
Many further possibilities of displaying equations exist. Some or shown in \path{TemplateDocumentation.pdf}
...
For complex subfigure constructs and correct alignment of the subcaption the \texttt{floatrow} provides powerful commands. These are described in \path{TemplateDocumentation.pdf}.
