...
% for futher simplification of showing commands a new command is defined
...
The standard command for citations is \texttt{\textbackslash{}cite} which may have a prenote argument for adding a page number or something similar. To show how a citation is typeset we cite here a book about LaTeX \cite[59]{companion}. Further commands such as \cs{parencite} \parencite{companion} and \cs{textcite} \textcite{companion} allow a different typeset of the citation. The resulting bibliography is printed out on \cpageref{sec:bibliography}. Refer to the biblatex manual for further details on citation commands and modifications on the printout and the section on biblatex in \url{TemplateDemo.pdf}.
...
The design and creating of complex tables is shown in much greater detail in \url{TemplateDemo.pdf} in the section about tables.
...
allows you to list the file \texttt{latextutorial.tex} in the directory \path{content/demo/} as done with the listing in the following \cref{sec:example:code}.
...
An index is created with the command \cs{index\{word\}} and can be grouped with 
\cs{index\{group!word\}}. The index created with this document is displayed on page~\pageref{sec:Index}. Within this document some index commands are grouped togther with the section headers of this tutorial.
