...
\ifbool{isMarginWidthDefined}{}{
 ...
 \lstinputlisting[style=demostyle]{\democodefile}
} % ifbool
}{} % ifcsdef
...
This template defines the commands \democs{doctextwidth} and \democs{doctextheight} which maintain their size even if the surrounding \democs{textwidth} changes. \vspace{0.5\baselineskip} \\
...
\begin{itemize}
\item In section~\ref{sec:tableStylesIntro} different style to create a table are shown: 
  \begin{itemize}
...
There a many ways to design a table in terms of its lines (grid), sizes, fonts and colors. Most of this can be regarded as personal taste. However the simplest on, the grid design, is regarded as a style which should be avoided by any means, since it makes it difficult for the eye to read the table.
...
This package unifies the application of a style to a table. The following styles are predefined: default, roman (serif instead of sans fonts), sansbold (bold header), sansboldbw (white text on black background)
...
List require a special correction to be not, or rather to be intended correct in a tabular cell. The same commands work in tabu tables, see section~\ref{sec:sec:tableitemliststabu}.
...
\begin{filecontents*}{\democodefile}
Lorem\index{Lorem} ipsum\index{ipsum} dolor\index{dolor} sit amet,
consectetuer adipiscing elit Nam dui ligula, fringilla a, euismod 
